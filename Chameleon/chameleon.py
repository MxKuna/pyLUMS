...
class Chameleon(DeviceOverZeroMQ):
    ...
    def createDock(self, parentWidget, menu=None):
        ...
        self.red_rectangle.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.wavelength_indicator.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.left_button.clicked.connect(
            lambda: self.open_shutter_fixed(not self._fixed_shutter_open)
        )
        ...
        self.right_button.clicked.connect(
            lambda: self.open_shutter_tunable(not self._tunable_shutter_open)
        )
        ...
        self.wavelength_set_button.clicked.connect(
            lambda: self.set_wavelength_with_safety()
        )
        ...
        for label, value in preset_buttons:
            ...
            preset_btn.clicked.connect(
                lambda _, v=value: self.set_wavelength_with_safety(v)
            )
        ...
        for i, (label, text_field) in enumerate(status_indicators):
            ...
            text_field.setFocusPolicy(Qt.FocusPolicy.NoFocus)  # Prevent focus
            text_field.setCursor(
                QCursor(Qt.CursorShape.ArrowCursor)
            )  # Normal cursor instead of hand
        ...
        self.checkbox_fixed.stateChanged.connect(
            lambda state: self.set_align_fixed(
                1 if state == Qt.CheckState.Checked else 0
            )
        )
        self.checkbox_tunable = QtWidgets.QCheckBox("TUNABLE")
        self.checkbox_tunable.stateChanged.connect(
            lambda state: self.set_align_tunable(
                1 if state == Qt.CheckState.Checked else 0
            )
        )
        ...
        parentWidget.addDockWidget(Qt.DockWidgetArea.TopDockWidgetArea, dock)
    ...
    def set_wavelength_with_safety(self, wavelength=None):
        ...
        try:
            ...
            QMessageBox.warning(
                None, "Input Error", "Please enter a valid integer wavelength."
            )
    ...
    def updateSlot(self, status):
        ...
        try:
            ...
            if status["laser"]["lasing"]:
                self.red_rectangle.setStyleSheet(
                    "background-color: darkred; color: white; font-weight: bold; font-size: 20px; border: 2px solid red; padding: 1px; border-radius: 10px;"
                )
            else:
                self.red_rectangle.setStyleSheet(
                    "background-color: gray; color: white; font-weight: bold; font-size: 20px; border: 2px solid darkgray; padding: 1px; border-radius: 10px;"
                )
    ...
    def update_state_info(self, s):
        ...
        color_dict = {
            "keyswitch": {
                1: "#90EE90",
                0: "#FFB6C1",
            },  # Light green for ON, light pink for OFF
            "tuning": {
                1: "#FFD700",
                0: "#90EE90",
            },  # Gold for Tuning, light green for Tuned
            "lasing": {
                1: "#90EE90",
                0: "#D3D3D3",
            },  # Tomato for Lasing, light gray for Not Lasing
        }

        # Update keyswitch text and color
        self.keyText.setText(dict["keyswitch"][s["keyswitch"]])
        self.keyText.setStyleSheet(
            f"QPushButton {{ background-color: {color_dict['keyswitch'][s['keyswitch']]}; color: black; font-weight: bold; text-align: center; }}"
        )
        ...
        if s["busy"] in ("OK", "Fixed Alignment Mode", "Variable Alignment Mode"):
            self.busyText.setStyleSheet(
                "QPushButton { background-color: #90EE90; color: black; font-weight: bold; text-align: center; }"
            )  # Light green for OK
        else:
            self.busyText.setStyleSheet(
                "QPushButton { background-color: #FFD700; color: black; font-weight: bold; text-align: center; }"
            )  # Gold for other states

        # Update tuning text and color
        self.tuningText.setText(dict["tuning"][s["tuning"]])
        self.tuningText.setStyleSheet(
            f"QPushButton {{ background-color: {color_dict['tuning'][s['tuning']]}; color: black; font-weight: bold; text-align: center; }}"
        )

        # Update lasing text and color
        self.lasingText.setText(dict["lasing"][s["lasing"]])
        self.lasingText.setStyleSheet(
            f"QPushButton {{ background-color: {color_dict['lasing'][s['lasing']]}; color: black; font-weight: bold; text-align: center; }}"
        )
